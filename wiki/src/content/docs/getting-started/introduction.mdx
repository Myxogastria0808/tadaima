...
tadaima is built on top of [greetd](https://sr.ht/~kennylevinsen/greetd/), a minimal and flexible login manager daemon for Linux. greetd handles the low-level authentication (via PAM) and session lifecycle, while exposing a simple IPC protocol over a Unix socket. tadaima implements this [greetd-ipc(7)](https://man.archlinux.org/man/greetd-ipc.7.en) protocol — sending JSON requests and receiving responses — so your greeter can authenticate users and start sessions without dealing with the socket or protocol details directly.
